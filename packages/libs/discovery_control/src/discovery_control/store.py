...
class ControlStore:
    def __init__(self, table_name: str | None = None, dynamodb_resource=None, ttl_days: int = 14):
        self.table_name = table_name or os.environ.get(
            DISCOVERY_CONTROL_TABLE_NAME, "discovery-control"
        )
    ...
    def create_run_if_absent(
        self, *, discovery_run_id: str, scrape_date: str, sitemap_raw_key: str
    ) -> bool:
        try:
            self.table.put_item(
                Item={
                    **self.run_key(discovery_run_id),
                    "item_type": "run",
                    "discovery_run_id": discovery_run_id,
                    "scrape_date": scrape_date,
                    "sitemap_raw_key": sitemap_raw_key,
    ...
    def seed_run(
        ...
        *,
        ...
        removed_spot_ids: list[str] | None = None,
    ) -> None:
        self.create_run_if_absent(
            discovery_run_id=discovery_run_id,
            scrape_date=scrape_date,
            sitemap_raw_key=sitemap_raw_key,
        )
        self.update_run_plan(
            ...
            expected_spot_count=expected_spot_count,
            added_count=expected_spot_count,
            removed_count=len(removed_spot_ids or []),
            existing_spot_count=0,
            status=RUN_STATUS_WAITING_FOR_SPOT_SCRAPES,
        )
        ...
        *,
        ...
        self.table.update_item(
            ...
            ExpressionAttributeValues={
                ":m": planner_manifest_key,
                ":e": expected_spot_count,
                ":a": added_count,
                ":r": removed_count,
                ":x": existing_spot_count,
                ":s": status,
                ":u": _isoformat(),
                ":ttl": self._ttl(),
            },
    ...
    def mark_spot_terminal(
        ...
        *,
        ...
        try:
            self.table.update_item(
                Key=self.spot_key(discovery_run_id, spot_id),
                UpdateExpression="SET " + ", ".join(sets),
                ConditionExpression="attribute_exists(pk) AND attribute_not_exists(terminal_status)",
        ...
        self.table.update_item(
            Key=self.run_key(discovery_run_id),
            UpdateExpression="ADD terminal_scrape_count :one, terminal_count :one, success_scrape_count :s, success_count :s, failed_scrape_count :f, failed_count :f SET updated_at=:u, expires_at=:ttl",
            ExpressionAttributeValues={
                ":one": 1,
                ":s": 1 if terminal_status == SPOT_STATUS_SUCCESS else 0,
                ":f": 1 if terminal_status == SPOT_STATUS_FAILED else 0,
                ":u": _isoformat(),
                ":ttl": self._ttl(),
            },
        )
    ...
    def mark_complete(self, discovery_run_id: str) -> None:
        self.table.update_item(
            Key=self.run_key(discovery_run_id),
            UpdateExpression="SET #s=:s, completed_at=:c, updated_at=:u, expires_at=:ttl",
            ExpressionAttributeNames={"#s": "status"},
            ExpressionAttributeValues={
                ":s": RUN_STATUS_COMPLETE,
                ":c": _isoformat(),
                ":u": _isoformat(),
                ":ttl": self._ttl(),
            },
