...
def _control_table():
    ...
    return dynamodb.Table(os.environ["DISCOVERY_CONTROL_TABLE_NAME"])


def test_discovery_diff_seeds_dynamo_and_enqueues_added_spots(s3, monkeypatch, lambda_context):
    bucket = "dataeng-squeegee-test-bucket"
    sqs_client = boto3.client("sqs", region_name="eu-west-2")
    spot_queue_url = sqs_client.create_queue(QueueName="spot-scraper")["QueueUrl"]
    catalog_queue_url = sqs_client.create_queue(QueueName="catalog-build")["QueueUrl"]
    ...
    run_item = _control_table().get_item(Key={"pk": "RUN#sitemap-1", "sk": "RUN"})["Item"]
    assert run_item["expected_spot_count"] == 2
    assert run_item["status"] == "waiting_for_spot_scrapes"

    messages = sqs_client.receive_message(QueueUrl=spot_queue_url, MaxNumberOfMessages=10)["Messages"]
    queued_spot_ids = sorted(json.loads(message["Body"])["spot_id"] for message in messages)
    assert queued_spot_ids == ["a", "b"]


def test_discovery_diff_removed_only_queues_catalog_build(s3, monkeypatch, lambda_context):
    bucket = "dataeng-squeegee-test-bucket"
    sqs_client = boto3.client("sqs", region_name="eu-west-2")
    spot_queue_url = sqs_client.create_queue(QueueName="spot-scraper-removed")["QueueUrl"]
    catalog_queue_url = sqs_client.create_queue(QueueName="catalog-build-removed")["QueueUrl"]
    monkeypatch.setenv("SPOT_SCRAPER_QUEUE_URL", spot_queue_url)
    monkeypatch.setenv("DISCOVERY_CATALOG_BUILD_QUEUE_URL", catalog_queue_url)
    ...
    run_item = _control_table().get_item(Key={"pk": "RUN#sitemap-2", "sk": "RUN"})["Item"]
    assert run_item["removed_spot_ids"] == ["gone"]
    assert run_item["status"] == "catalog_build_ready"

    messages = sqs_client.receive_message(QueueUrl=catalog_queue_url, MaxNumberOfMessages=10)["Messages"]
    assert json.loads(messages[0]["Body"]) == {"discovery_run_id": "sitemap-2"}
