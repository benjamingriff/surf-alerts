...
def test_discovery_completion_emits_manifest_when_terminal_success_and_failure_count_met(s3, lambda_context):
    ...
    _write_json(
        s3,
        bucket,
        "control/completions/discovery_spot_scrapes/date=2026-03-09/discovery_run_id=run-2/spot_id=a.json.gz",
        {"spot_id": "a", "raw_key": "raw/spot_report/spot_id=a/..."},
    )
    _write_json(
        s3,
    ...
    assert processing_manifest["raw_keys"] == ["raw/spot_report/spot_id=a/..."]
