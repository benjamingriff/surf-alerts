...
- Backups: one-day automated retention
- Credentials: generated by AWS/CDK in Secrets Manager as `surf-alerts/postgres/app-credentials`
- Application URL: manually stored as SSM SecureString `/surf-alerts/rds/postgres-url`
- Runtime environment variable: `POSTGRES_URL_PARAMETER_NAME=/surf-alerts/rds/postgres-url`
- Networking: publicly accessible with public TCP/5432 ingress for manual learning-phase administration

See [Manual RDS Setup](../operations/rds-setup.md) for migration order, connection URL creation, SSM storage, and temporary security notes.
